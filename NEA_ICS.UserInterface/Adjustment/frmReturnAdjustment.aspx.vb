...
''' <summary>
''' Code behind for frmReturnAdjustment Page;
''' 28Feb12;
''' </summary>
''' <remarks>
''' CHANGE LOG:
...
''' </remarks>
Partial Public Class frmReturnAdjustment
    Inherits clsCommonFunction

#Region " Page Control "
    Private Message As String = EMPTY
    Private AdjustReturnItem As AdjustReturnItem
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not Page.IsPostBack Then
                ...
                If CacheListIsEmpty(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), GetType(AdjustDetails)) Then GetAdjustList(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), Session(ESession.StoreID.ToString), RETURNED, ALL)
                ...
                Dim ClosedStocksReturn As List(Of AdjustDetails) = DirectCast(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), List(Of AdjustDetails)).FindAll(Function(item As AdjustDetails) item.Status = CLOSED)
                BindDropDownList(ddlRptDocumentNo, ClosedStocksReturn, "AdjustID", "AdjustID")
            End If
...
#Region " New Tab "
    Private Function ValidReturn() As Boolean
        Try
            ...
            If AdjustDte = DateTime.MinValue Then
                Throw New ApplicationException(GetMessage(messageID.NotIsDate, "Return Date"))
            End If
            If AdjustDte > Today Then
                Throw New ApplicationException(GetMessage(messageID.MoreLessThan, , , "Return Date", Today.ToString("dd/MM/yyyy"), "earlier or same as"))
            End If
            If Not (IsWithinFinanceCutoffDate(Cache(Session(ESession.StoreID.ToString) & "dtCommon"), Session(ESession.StoreID.ToString), AdjustDte)) Then
                Throw New ApplicationException(GetMessage(messageID.NotInFinancial, "Return Date"))
            End If
            ...
            ' validate Return Ref. ID is unique
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            ...
            ' validate mandatory fields; when Return Ref. not entered, system will generates a unique number instead
            ...
            If ValidReturn() Then
                ...
                If DetailsList.Count > 0 Then
                    ...
                    ' Assign the Return Ref ID for adding new item
                    Adjust.AdjustID = AdjustIDReturn

                    ''If Message = EMPTY Then
                    Message = GetMessage(messageID.Success, "Saved", String.Format("Stock Return[{0}]", AdjustIDReturn))
    ...
    'Private Function LoadReturnCostDetails(ByVal requestId As String, ByVal itemCodeId As String) As AdjustItemDetails
    ...
    '                                            , requestId _
    '                                            )
    '        'AdjustDetails = DirectCast(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), List(Of AdjustDetails)).Find(Function(i As AdjustDetails) i.AdjustID = txtLocateAdjustID.Text.Trim())
    ...
    '    IssueItem = DetailsList.Find(Function(i As IssueItemDetails) i.RequestItemID = requestId)

    '    ' Issue qty is stored as negative, hence need to convert to positive value
    '    If (DetailsItem.Qty <> 0) Then ReturnUnitCost = IssueItem.TotalCost / IssueItem.Qty
    ...
    '    DetailsItem.ItemReturn = IssueItem.TranID
    '    DetailsItem.BalanceQty = IssueItem.BalanceQty
    '    DetailsItem.MaxLevel = (-IssueItem.Qty)
    '    ''UpdateGTotal(item.TotalCost)

    '    'DetailsItem.Mode = UPDATE

    '    'DirectCast(AdjustReturnItem.FindControl("hfMode"), HiddenField).Value = DetailsItem.Mode
    '    DirectCast(AdjustReturnItem.FindControl("hfTranID"), HiddenField).Value = IssueItem.TranID
    ...
    '    DirectCast(AdjustReturnItem.FindControl("hfItemReturn"), HiddenField).Value = IssueItem.TranID
    '    DirectCast(AdjustReturnItem.FindControl("hfBalanceQty"), HiddenField).Value = IssueItem.BalanceQty
    '    DirectCast(AdjustReturnItem.FindControl("hfMaxLevel"), HiddenField).Value = (-IssueItem.Qty)

    '    'DirectCast(AdjustReturnItem.FindControl("hfOrgAdjustQty"), HiddenField).Value = item.Qty
    '    DirectCast(AdjustReturnItem.FindControl("hfOrgTotalCost"), HiddenField).Value = IssueItem.TotalCost
    '    'DirectCast(AdjustReturnItem.FindControl("hfOrgRemarks"), HiddenField).Value = item.Remarks
    ...
    '    'DirectCast(AdjustReturnItem.FindControl("txtAdjustQty"), TextBox).Text = IIf(item.Qty <> 0, (item.Qty).ToString("0.00"), EMPTY)

    '    'DirectCast(AdjustReturnItem.FindControl("txtReceiveQty"), TextBox).Text = IIf(item.Qty <> 0, (item.Qty).ToString("0.00"), EMPTY)

    '    'DirectCast(AdjustReturnItem.FindControl("txtRemarks"), TextBox).Text = item.Remarks
...
#Region " Locate Tab "
    ...
    Private Sub btnLocateGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLocateGo.Click
        Try
            ...
            GetAdjustListBySearch(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), Session(ESession.StoreID.ToString), ddlLocateConsumerSearch.SelectedValue, txtLocateAdjustID.Text, RETURNED, ddlLocateStatus.SelectedValue)
            gdvLocate.DataSource = Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList)
            gdvLocate.DataBind()
            ...
            'Dim Adjust As AdjustDetails = DirectCast(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), List(Of AdjustDetails)).Find(Function(i As AdjustDetails) i.AdjustID = txtLocateAdjustID.Text.Trim())
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            Select Case CType(e.Row.FindControl("hidStatus"), HiddenField).Value.ToUpper
                ...
                Case REJECTED
                    ...
                    'Case PENDING
                    '    CType(e.Row.FindControl("lblStatus"), Label).Text = "APPROVED" ' set APPROVED as status if Return is pending for Receive
    ...
    Private Sub gdvLocate_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gdvLocate.SelectedIndexChanged
        Try
            ...
            Dim Adjust As AdjustDetails = DirectCast(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList), List(Of AdjustDetails)).Find(Function(i As AdjustDetails) i.AdjustID = lblLocateAdjustID.Text)
            If Adjust Is Nothing Then Throw New NullReferenceException
            Dim AdjustItemList As List(Of AdjustItemDetails)
            Dim ReturnUnitCost As Double = 0.0
            Dim IssueDetailsList As List(Of IssueItemDetails)
            ...
            For Each item In AdjustItemList
                ...
                'Dim DetailsItemCost As New AdjustItemDetails
                'DetailsItemCost = LoadReturnCostDetails(lblLocateRequestID.Text, item.StockItemID)
                ...
                If DetailsItem.Qty > 0 Then
                    ReturnUnitCost = -CDec(DetailsItem.TotalCost / DetailsItem.Qty)
                End If

                'ReturnUnitCost = IIf(DetailsItem.Qty > 0, -CDec(DetailsItem.TotalCost / DetailsItem.Qty), 0.0)
                ''UpdateGTotal(item.TotalCost)
    ...
    Protected Sub btnLocateSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLocateSave.Click
        Try
            ...
            ' validate date
            If (AdjustDte > Today) Then Throw New ApplicationException(GetMessage(messageID.MoreLessThan, , , "Return Date", Today.ToString("dd/MM/yyyy"), "earlier or same as"))
            If Not (IsWithinFinanceCutoffDate(Cache(Session(ESession.StoreID.ToString) & "dtCommon"), Session(ESession.StoreID.ToString), AdjustDte)) Then Throw New ApplicationException(GetMessage(messageID.NotInFinancial, "Stock Return Date"))

            ' check if Return is updated
            ...
            If txtReceiveDate.Text <> EMPTY Then
                ...
                Message = GetMessage(messageID.Success, "Received", String.Format("Stock Return Request[{0}]", lblLocateAdjustID.Text))
            ...
            If DetailsList.Count > 0 Or AdjustUpdated Then
                ...
                If Message = EMPTY Then
                    ...
                    Else
                        Message = GetMessage(messageID.Success, "Edited", String.Format("Stock Return[{0}]", Adjust.AdjustID))
                    End If


                    ' Add edited Return to list and rebind (if needed)
                    EditCache(Cache(Session(ESession.StoreID.ToString) & ECache.AdjustInList) _
                              , GetType(AdjustDetails) _
                              , Adjust _
                              , True _
    ...
    Protected Sub btnLocateApprove_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLocateApprove.Click
        ...
        Message = GetMessage(messageID.Success, "Approved", String.Format("Store Return[{0}]", lblLocateAdjustID.Text))
    ...
    Private Sub btnLocateReject_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLocateReject.Click
        ...
        Message = GetMessage(messageID.Success, "Rejected", String.Format("Stock Return[{0}]", lblLocateAdjustID.Text))
    ...
    Private Sub btnLocateDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLocateDeleteAll.Click
        Try
            ...
            If Message = EMPTY Then
                ...
                Message = GetMessage(messageID.Success, "deleted", "Return")
            Else
...
#Region " Print Tab "
    Protected Sub btnPDF_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPDF.Click
        ...
        Dim p4 As New ReportParameter("ReportTitle", "Stock Item Return Report")
        ...
        Response.AddHeader("content-disposition", "attachment;filename=StockReturn.pdf")
    ...
    Protected Sub btnExcel_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExcel.Click
        ...
        Response.AddHeader("content-disposition", "attachment;filename=StockReturn.xls")
        Response.BinaryWrite(bytValue)
        Response.Flush()
        Response.End()
    End Sub
    ...
    Protected Sub ObjectDataSource1_Selecting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceSelectingEventArgs) Handles ObjectDataSource1.Selecting
        ...
        e.InputParameters("type") = RETURNED
...
#Region " Sub Procedures and Functions "
    ...
    Protected Friend Sub ViewStock(ByVal uniqueID As String, ByVal stockItemID As String, ByVal stockItemDesc As String, ByVal stockItemUOM As String, ByVal adjustQtyDiff As Decimal, ByVal totalDiff As Double)
        Try
            ...
            ' Return inward, hence add to the current value
    ...
    Protected Friend Sub CancelAdjustItem(ByVal sender As Object, ByVal e As EventArgs)
        Try
            Dim AdjustReturnItem As New AdjustReturnItem
            AdjustReturnItem = DirectCast(FindControl(sender), AdjustReturnItem)
            If tbcAdjustItem.ActiveTab.ID = NEWTAB Then
                DCP.Controls.Remove(FindControl(sender))
                uplUserControl.Update()
                ' remove stockcode from returned list
                DirectCast(ViewState(EViewState.AdjustedStockItemID), List(Of String)).Remove(DirectCast(AdjustReturnItem.FindControl("lblStockCode"), Label).Text)

            ElseIf tbcAdjustItem.ActiveTab.ID = LOCATETAB Then
                If DirectCast(AdjustReturnItem.FindControl("hfMode"), HiddenField).Value = INSERT Then
                    DCPLocate.Controls.Remove(FindControl(sender))
                Else
                    DirectCast(AdjustReturnItem.FindControl("hfMode"), HiddenField).Value = DELETE
                    AdjustReturnItem.Visible = False
                End If
                ' remove stockcode from adjusted list
                DirectCast(ViewState(EViewState.AdjustedStockItemIDLocate), List(Of String)).Remove(DirectCast(AdjustReturnItem.FindControl("lblStockCode"), Label).Text)
    ...
    Protected Function RetrieveDCPInfo(ByVal UserControlName As String, ByVal adjustType As String) As List(Of AdjustItemDetails)
        ...
        For i As Integer = 1 To ViewState(UserControlName)
            ...
            If AdjustReturnItem IsNot Nothing Then
                ...
                DetailsItem.AdjustItemID = CInt(DirectCast(AdjustReturnItem.FindControl("hfAdjustReturnItemID"), HiddenField).Value)
                ...
                DetailsItem.Mode = DirectCast(AdjustReturnItem.FindControl("hfMode"), HiddenField).Value
                ''If DetailsItem.Mode <> INSERT Then
                ''    ' Set Item Mode
                ''    DetailsItem.Mode = GetItemMode(DetailsItem.TranID _
                ''                                   , DetailsItem.Qty _
                ''                                   , CDec(DirectCast(AdjustReturnItem.FindControl("hfOrgAdjustQty"), HiddenField).Value) _
        ...
        For i As Integer = 1 To ViewState(UserControlName)
            ...
            If AdjustReturnItem IsNot Nothing Then
                ...
                If DirectCast(AdjustReturnItem.FindControl("checkReceived"), CheckBox).Checked = True Then
                    DetailsItem.Status = CLOSED ' return is received
                    DetailsItem.Mode = UPDATE
                    'update return qty to actual return received qty
                    Value = Trim(DirectCast(AdjustReturnItem.FindControl("txtReceiveQty"), TextBox).Text)
                    If IsNumeric(Value) Then DetailsItem.Qty = CDec(Value)
                Else
                    DetailsItem.Status = OPEN ' return is amended
                    Value = Trim(DirectCast(AdjustReturnItem.FindControl("txtAdjustQty"), TextBox).Text)
                    If IsNumeric(Value) Then DetailsItem.Qty = CDec(Value)
                End If
