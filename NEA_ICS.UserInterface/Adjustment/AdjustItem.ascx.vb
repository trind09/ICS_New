...
''' </remarks>
Partial Public Class AdjustItem
    ...
    Protected Sub txtTotalCost_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtTotalCost.TextChanged
        ...
        Catch ex As ApplicationException
            txtTotalCost.Text = IIf(CDbl(hfOrgTotalCost.Value) > 0.0, CDec(hfOrgTotalCost.Value).ToString("0.00"), EMPTY)
            _message = ex.Message

        Catch ex As Exception
            txtTotalCost.Text = IIf(CDbl(hfOrgTotalCost.Value) > 0, CDec(hfOrgTotalCost.Value).ToString("0.00"), EMPTY)
    ...
    Protected Sub btnViewDetails_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnViewDetails.Click
        ...
        If IsNumeric(txtTotalCost.Text) Then
    ...
    Private Sub SetMode()
        ...
        If (hfMode.Value = EMPTY Or hfMode.Value = UPDATE) Then
            If IsNumeric(txtAdjustQty.Text) Then AdjustQty = CDec(txtAdjustQty.Text)
            If IsNumeric(txtTotalCost.Text) Then TotalCost = CDec(txtTotalCost.Text)

            If (CDec(hfOrgAdjustQty.Value) <> AdjustQty _
                Or CDbl(hfOrgTotalCost.Value) <> TotalCost _
