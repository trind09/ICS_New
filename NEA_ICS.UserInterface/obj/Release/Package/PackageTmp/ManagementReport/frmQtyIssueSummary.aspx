<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="frmQtyIssueSummary.aspx.vb"
    Inherits="NEA_ICS.UserInterface.frmQtyIssuedSummary" ValidateRequest="true" %>
...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function setStockCode(checkStockCode) {
            ...
            var reqStockCodefrom = document.getElementById("reqStockCodeFrom");
            var reqStockCodeto = document.getElementById("reqStockCodeTo");
            var comStockCode = document.getElementById("comStockCode");

            if (checkStockCode.checked == true) {
                StockCodefrom.disabled = true;
                StockCodeto.disabled = true;
                reqStockCodefrom.isvalid = true;
                reqStockCodeto.isvalid = true;
                comStockCode.isvalid = true;
                ValidatorEnable(reqStockCodefrom, false);
                ValidatorEnable(reqStockCodeto, false);
                ValidatorEnable(comStockCode, false);
            }
            else {
                StockCodefrom.disabled = false;
                StockCodeto.disabled = false;
                reqStockCodefrom.isvalid = false;
                reqStockCodeto.isvalid = false;
                comStockCode.isvalid = false;
                ValidatorEnable(reqStockCodefrom, true);
                ValidatorEnable(reqStockCodeto, true);
                ValidatorEnable(comStockCode, true);
            }
        }
        ...
        function setTransDate(checkTransDate) {
            ...
            if (checkTransDate.checked == true) {
                ...
                reqDateFrom.isvalid = true;
                reqDateTo.isvalid = true;
                comDate.isvalid = true;
                ValidatorEnable(reqDateFrom, false);
                ValidatorEnable(reqDateTo, false);
                ValidatorEnable(comDate, false);
            }
            else {
                txtDateFrom.disabled = false;
                txtDateTo.disabled = false;
                reqDateFrom.isvalid = false;
                reqDateTo.isvalid = false;
                comDate.isvalid = false;
                ValidatorEnable(reqDateFrom, true);
                ValidatorEnable(reqDateTo, true);
                ValidatorEnable(comDate, true);
            }
        }
    </script>
        ...
        <table class="moduleTitle" width="98%" cellspacing="1" cellpadding="1">
            <tr>
                <td class="moduleTitleBorder">
                    Management Report > Quantity Issue Summary
                </td>
            </tr>
        </table>
        <br />
            ...
            <asp:UpdatePanel ID="uplItem" runat="server">
                <ContentTemplate>
                    ...
                    <rsweb:ReportViewer ID="rvr" runat="server" Font-Size="8pt" Height="400px" Visible="False"
                        ...
                        <LocalReport ReportPath="ManagementReport\rptMR008QuantityIssueSummary.rdlc">
                            <DataSources>
                                <rsweb:ReportDataSource DataSourceId="ObjectDataSource1" Name="MR008QuantityIssueSummaryDetails" />
                            </DataSources>
                        </LocalReport>
                    </rsweb:ReportViewer>
                    <asp:ObjectDataSource ID="ObjectDataSource1" runat="server" SelectMethod="GetMR008QuantityIssueSummary"
